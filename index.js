...
const { getTracks, getPreview } = require('spotify-url-info')(require('undici').fetch);
...
help_cat1_val: "`/play` - Reproducir música\n`/playalbum` - Añadir álbum de Spotify\n`/skip` - Saltar tema actual\n`/stop` - Detener bot\n`/song` - Ver panel del tema actual\n`/queue` - Ver la cola de temas\n`/shuffle` - Mezclar la cola\n`/remove` - Quitar un tema\n`/lyrics` (Premium) - Ver la letra",        help_cat2: "Automatización",
    ...
    'en': {
        ...
        help_cat1: "Playback",
help_cat1_val: "`/play` - Reproducir música\n`/playalbum` - Añadir álbum de Spotify\n`/skip` - Saltar tema actual\n`/stop` - Detener bot\n`/song` - Ver panel del tema actual\n`/queue` - Ver la cola de temas\n`/shuffle` - Mezclar la cola\n`/remove` - Quitar un tema\n`/lyrics` (Premium) - Ver la letra",        help_cat2: "Automation",
...
const SPAM_WORDS = [
    ...
    '8d', '3d', '16d', 'audio espacial', 'nightcore', 'sped up', 'slowed', 'reverb',
    'sub', 'español', 'espanol', 'traducida', 'letra', 'lyrics', 'amv', 'fmv', 'bass boosted'
];
...
async function getTidalTracks(url) {
    try {
        ...
        let allTracks = [];
        const mobileTokens = ['gsFXkJqGrUNoYMQPZe4k3WKwijnrp8iGSwn3bApe', '4zx46pyr9o8qZNRw'];
        
        if (isTrack) {
            
            const res = await request(`https://api.tidal.com/v1/tracks/${tidalId}?countryCode=US`, { headers: { 'x-tidal-token': mobileTokens[0] } });
            ...
            while (hasMore && allTracks.length < 500) { 
                const res = await request(`https://api.tidal.com/v1/${type}/${tidalId}/items?limit=${limit}&offset=${offset}&countryCode=US`, {
        ...
        return allTracks.filter(t => t && t.title).map(t => ({
            name: t.title,
            artist: t.artist?.name || t.artists?.[0]?.name || "Desconocido",
            id: t.id,
            thumbnail: t.album?.cover ? `https://resources.tidal.com/images/${t.album.cover.replace(/-/g, '/')}/640x640.jpg` : 'https://i.imgur.com/Q2v1vV7.png'
        }));
    } catch (e) {
...
function getQueue(guildId) {
    if (!globalQueues.has(guildId)) {
        ...
        globalQueues.set(guildId, {
            songs: [], history: [], titleHistory: [], playedHistory: [], compareHistory: [],
            ...
            currentProcess: null, volume: 100 
        });
    }
...
const commandsDef = [
    ...
    { name: 'autoplay', description: 'Alternar modo de reproduccion basado en Likes (Strict)' },
    { name: 'discovery', description: 'Alternar modo de reproduccion basado en Generos (Discovery)' },
    ...
    { name: 'playalbum', description: 'Buscar y reproducir un álbum oficial', options: [{ type: 3, name: 'query', description: 'Nombre del álbum y artista', required: true }] }];
...
const getPlaybackRows = (q, l) => {
    const row1 = new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId('pause_resume').setLabel(q.player.state.status === AudioPlayerStatus.Paused ? "Resume" : "Pause").setStyle(ButtonStyle.Secondary),
        new ButtonBuilder().setCustomId('skip_song').setLabel("Skip").setStyle(ButtonStyle.Secondary),
        new ButtonBuilder().setCustomId('stop_song').setLabel("Stop").setStyle(ButtonStyle.Secondary)
    );
    const row2 = new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId('shuffle_queue').setLabel("Shuffle").setStyle(ButtonStyle.Secondary),
        new ButtonBuilder().setCustomId('toggle_autoplay').setLabel("AutoPlay").setStyle(q.autoplay ? ButtonStyle.Success : ButtonStyle.Secondary),
        new ButtonBuilder().setCustomId('like_song').setLabel("Guardar").setStyle(ButtonStyle.Success)
    );
    return [row1, row2];
};
...
async function fetchLrclib(title, artist) {
    try {
        ...
        const res = await request(`https://lrclib.net/api/search?${query.toString()}`);
        const data = await res.body.json();
...
const getCompareString = (name) => {
    ...
    return name.toLowerCase()
        ...
        .replace(/(?:remaster|mix|edit|version|live|demo|official|video|audio|hq|hd|4k)\b/gi, '') 
        .replace(/[0-9]{4}/g, '') 
        .replace(/[^a-z]/g, ''); 
};
...
const createSong = (video, artistOverride = null, requester = 'Motor IA', requesterAvatar = null) => {
    ...
    const isWebId = video.videoId && /^(itunes|spotify|tidal)_/.test(String(video.videoId));

    const safeUrl = video.url || (isWebId ? '' : `https://www.youtube.com/watch?v=${video.videoId}`);
    ...
    if (!thumb || thumb.includes('ui-avatars')) thumb = 'https://i.imgur.com/Q2v1vV7.png';
...
async function startProgressInterval(guildId, vibrantColor) {
    ...
    q.progressInterval = setInterval(async () => {
        ...
        if (!q.playing || !q.lastSong || q.player.state.status === AudioPlayerStatus.Paused) return;
        ...
        if (q.voiceChannel) {
            ...
            if (activeUserIds.length > 0) {
                ...
                `;
                pool.query(upsertTime, [activeUserIds]).catch(e => console.error("[STATS TIME ERROR]:", e.message));
            }
        }
        ...
        if (q.currentMessage) {
            ...
            if(q.filter && q.filter !== 'clear') embedPlay.setFooter({ text: `Filtro Activo: ${q.filter}` });

            q.currentMessage.edit({ embeds: [embedPlay], components: rows }).catch(() => {
...
async function playNext(guildId) {
        ...
        if ((q.autoplay || q.discovery) && q.lastSong) {
            try {
                ...
                if (!isPrem && q.autoplayCount >= 5) {
                    q.autoplay = false; q.discovery = false; q.playing = false;
                    if (q.textChannel) q.textChannel.send({ embeds: [UI.info("SYSTEM", "Límite de Autoplay alcanzado.")] }).catch(()=>{});
                    return;
                }
                ...
                if (!activeModes.includes(q.currentBatchType) || q.batchCount >= 5) {
                    q.currentBatchType = activeModes[Math.floor(Math.random() * activeModes.length)];
                ...
                let targetQuery = "";
                let expectedArtist = "";
                let artistFallback = q.lastSong.realArtist || q.lastSong.artist || "Music";
                ...
                if (q.currentBatchType === "DISCOVERY") {
                    ...
                    try {
                        if (!q.currentBatchSeed) {
                            const trackInfo = await request(`https://itunes.apple.com/search?term=${encodeURIComponent(artistFallback)}&entity=song&limit=1`).then(r => r.body.json());
                            q.currentBatchSeed = (trackInfo.results && trackInfo.results.length > 0) ? trackInfo.results[0].primaryGenreName : "Pop";
                            console.log(`[DISCOVERY] Nuevo género fijado: ${q.currentBatchSeed}`);
                        }

                        const genreRes = await request(`https://itunes.apple.com/search?term=${encodeURIComponent(q.currentBatchSeed)}&entity=song&limit=50`).then(r => r.body.json());
                        
                        
                        const validGenreTracks = genreRes.results.filter(t => {
                            const name = t.trackName.toLowerCase();
                            const art = t.artistName.toLowerCase();
                            return !name.includes('cover') && !name.includes('karaoke') && !name.includes('tribute') && !art.includes('tribute');
                        });

                        const unheard = validGenreTracks.filter(t => !q.titleHistory.includes(cleanSongName(t.trackName)));
                        const randomTrack = unheard.length > 0 ? unheard[Math.floor(Math.random() * unheard.length)] : (validGenreTracks[0] || genreRes.results[0]);
                    ...
                    try {
                        if (!q.currentBatchSeed) {
                            ...
                            let artistPool = [];
                            if (activeUserIds.length > 0) {
                                const { rows } = await pool.query('SELECT artist FROM likes WHERE user_id = ANY($1::varchar[]) GROUP BY artist', [activeUserIds]);
                                artistPool = rows.map(r => r.artist);
                            }
                            if (artistPool.length === 0 && q.playedHistory && q.playedHistory.length > 0) {
                                artistPool = [...new Set(q.playedHistory.slice(-10).map(s => s.artist))];
                            }

                            q.currentBatchSeed = artistPool.length > 0 ? artistPool[Math.floor(Math.random() * artistPool.length)] : artistFallback;
                            console.log(`[STRICT] Nuevo artista fijado: ${q.currentBatchSeed}`);
                        }

                        expectedArtist = q.currentBatchSeed;
                        const itunesRes = await request(`https://itunes.apple.com/search?term=${encodeURIComponent(q.currentBatchSeed)}&entity=song&attribute=artistTerm&limit=50`).then(r => r.body.json());
                        
                        if (itunesRes.results && itunesRes.results.length > 0) {
                            
                            const strictTracks = itunesRes.results.filter(t => {
                                const isSameArtist = t.artistName.toLowerCase().includes(q.currentBatchSeed.toLowerCase()) || q.currentBatchSeed.toLowerCase().includes(t.artistName.toLowerCase());
                                const isNotCover = !t.trackName.toLowerCase().includes('cover') && !t.trackName.toLowerCase().includes('tribute') && !t.artistName.toLowerCase().includes('tribute');
                                return isSameArtist && isNotCover;
                            });

                            const unheard = strictTracks.filter(t => !q.titleHistory.includes(cleanSongName(t.trackName)));
                            const randomTrack = unheard.length > 0 ? unheard[Math.floor(Math.random() * unheard.length)] : (strictTracks[0] || itunesRes.results[0]);
                ...
                if (searchRes && searchRes.videos.length > 0) {
                    ...
                    let validVideos = searchRes.videos.filter(v => {
                        ...
                        if (vTitle.includes('cover') || vTitle.includes('tribute') || vTitle.includes('live')) return false;

                        
                        const expArtistLower = expectedArtist.toLowerCase().split(' ')[0]; 
                        const isOfficialChannel = vAuthor.endsWith('vevo') || vAuthor.endsWith('- topic') || vAuthor.includes('official') || vAuthor.includes(expArtistLower);
                    ...
                    if (validVideos.length === 0) {
                        validVideos = searchRes.videos.filter(v => !q.history.includes(v.videoId) && !SPAM_WORDS.some(sw => v.title.toLowerCase().includes(sw)) && !v.title.toLowerCase().includes('cover'));
                    }

                    let nextVideo = validVideos.length > 0 ? validVideos[0] : searchRes.videos[0];
                    
                    q.autoplayCount++;
                    q.batchCount++; 
                    
                    
                    let autoSong = createSong(nextVideo, expectedArtist, i18n[l].bot_magic, null);
                    ...
                    console.log(`[MOTOR] Sin resultados en YouTube. Forzando rotación de lote...`);
                    q.batchCount = 5; 
                    return playNext(guildId); 
                }
            } catch (e) { 
    ...
    if (song.videoId && /^(itunes|spotify|tidal)_/.test(String(song.videoId))) {
        try {
            const searchRes = await yts(`${song.title} ${song.artist} official audio`);
            if (searchRes && searchRes.videos.length > 0) {
                ...
                console.log(`[TRADUCTOR] No se encontró audio en YT para ${song.title}`);
                q.songs.shift();
                return playNext(guildId);
            }
        } catch(e) {
    ...
    if (!song.durationSec || song.durationSec === 0) {
        ...
        } catch(e) { console.error("[METADATA HEALER ERROR]:", e.message); }
    }
    ...
    if (q.voiceChannel && !song.isTrivia) {
        ...
        if (activeUserIds.length > 0) {
            ...
            if (!song.isAutoplay) {
                ...
                `;
                pool.query(upsertSongs, [activeUserIds]).catch(e => console.error("[STATS SONGS ERROR]:", e.message));
            }
            
            const historyQuery = `INSERT INTO user_history (user_id, video_id, title, artist) SELECT unnest($1::varchar[]), $2, $3, $4`;
            pool.query(historyQuery, [activeUserIds, song.videoId, song.title, song.artist]).catch(()=>{});
            
            activeUserIds.forEach(id => {
                io.emit('history_updated', { userId: id, videoId: song.videoId, title: song.title, artist: song.artist, playedAt: new Date().toISOString() });
            });
            ...
            if (!song.isTrivia) {
                ...
                `, [activeUserIds, song.videoId, song.title]).catch(e => console.log("Stats Play Error:", e.message));
            }
        }
    }
...
async function startStream(song, guildId, l) {
    ...
    try {
        ...
        ytdlpProcess.on('close', (code) => { if (code !== 0 && code !== null && q.playing) q.player.stop(); });
        ...
        if (!song.isTrivia) {
            ...
            `, [song.videoId, song.title]).catch(e => console.log("Stats Play Error:", e.message));
        }
        ...
        (async () => {
            try {
                ...
                let palette = {};
                try { palette = await Vibrant.from(imgBuffer).getPalette(); } catch (error) {}
                ...
                if (song.isTrivia) {
                    ctx.fillStyle = '#1e1f22'; ctx.fillRect(imgX, imgY, imgSize, imgSize); ctx.restore(); ctx.save();
                ...
                } else { ctx.drawImage(mainImage, imgX, imgY, imgSize, imgSize); }
                ctx.restore();

                const textX = imgX + imgSize + 60; const maxWidth = 1280 - textX - 40; 
                ...
                const words = displayTitle.split(' '); let line = ''; let lines = [];
                ...
                for(let i = 0; i < Math.min(lines.length, 2); i++) {
                    ...
                    ctx.fillText(textToDraw, textX, currentY); currentY += 85; 
                }
                ...
                if (q.currentMessage) {
                    await q.currentMessage.edit({ 
                        embeds: [embedPlay], 
                        components: rows, 
                        files: [attachment] 
                    }).catch(() => { q.currentMessage = null; });
                } 
...
client.on(Events.InteractionCreate, async interaction => {
    ...
    try {
        ...
        if (interaction.isButton()) {
            ...
            if (interaction.customId === 'skip_song') {
                ...
                if (q.lastSong && activeUserIds.length > 0) {
                    ...
                    `, [activeUserIds, q.lastSong.videoId, q.lastSong.title]).catch(()=>{});
                }
            ...
            if (interaction.customId === 'toggle_autoplay') {
                if (!await checkPremium(guildId)) {
                    return interaction.reply({ embeds: [UI.info("ACCESO", i18n[l].premium_only)], flags: [MessageFlags.Ephemeral] });
                }
                ...
                const statusMsg = q.autoplay ? "AUTOPLAY: ENABLED" : "AUTOPLAY: DISABLED";
                
                
                if (q.textChannel) {
                    q.textChannel.send({ embeds: [UI.success(statusMsg)] })
                        .then(m => setTimeout(() => m.delete().catch(()=>{}), 5000)).catch(()=>{});
                }
                
                io.emit("track_added", statusMsg);
                if (q.autoplay && !q.playing && q.songs.length === 0 && q.lastSong) playNext(guildId);
                return;
            }
            ...
            if (interaction.customId === 'shuffle_queue') {
                if (!await isAuthorized(interaction, q)) return interaction.reply({ embeds: [UI.error(i18n[l].dj_required)], flags: MessageFlags.Ephemeral });
                if (q.songs.length < 3) return interaction.reply({ embeds: [UI.error(i18n[l].queue_insufficient)], flags: MessageFlags.Ephemeral }).then(m => setTimeout(() => m.delete().catch(()=>{}), 5000));
                ...
                await interaction.reply({ embeds: [UI.success(i18n[l].queue_shuffled)], flags: MessageFlags.Ephemeral }).then(m => setTimeout(() => m.delete().catch(()=>{}), 5000));
                return;
            }
            ...
            if (interaction.customId === 'stop_song') {
                ...
                q.songs = []; q.history = []; q.titleHistory = []; 
                q.autoplay = false; q.discovery = false; q.autoplayCount = 0; q.batchCount = 0; q.lastSong = null;
                ...
                await interaction.reply({ embeds: [UI.info(i18n[l].connection, i18n[l].stopped)], flags: [MessageFlags.Ephemeral] }).then(m => setTimeout(() => m.delete().catch(()=>{}), 5000));
                return;
            }
        ...
        if (["play", "join", "search", "playliked", "playlist", "trivia", "radio", "playalbum"].includes(command)) {
            ...
            if (!permissions.has('Connect') || !permissions.has('Speak')) {
                await interaction.editReply({ embeds: [UI.error("No tengo permisos suficientes para unirme o hablar en ese canal de voz.")] });
                setTimeout(() => interaction.deleteReply().catch(()=>{}), MSG_LIFETIME);
                return;
            }
        ...
        if (command === "play") {
            ...
            if (query.includes("spotify.com")) {
                try {
                    ...
                    const defaultThumb = preview?.image || 'https://i.imgur.com/Q2v1vV7.png';

                    const tracks = await getTracks(query);
                    for (const track of tracks) {
                        const artist = track.artist || (track.artists ? track.artists[0].name : "Unknown");
                        
                        const thumb = track.album?.images?.[0]?.url || track.coverArt?.sources?.[0]?.url || defaultThumb;
                ...
                try {
                    ...
                    if (tracks.length === 0) return interaction.editReply({ embeds: [UI.error("No se pudieron extraer las pistas de este enlace de Tidal.")] });
                    ...
                    for (const track of tracks) {
                        q.songs.push({
                            ...
                            requester: userName,
                            requesterAvatar: userAvatar,
                            hasRealCover: track.thumbnail && track.thumbnail !== 'https://i.imgur.com/Q2v1vV7.png'
                        });
                    }
                    interaction.editReply({ embeds: [UI.success(`${i18n[l].added_queue}\n> **${tracks.length} pistas de Tidal**`)] });
                    if (!q.playing) playNext(guildId);
                } catch (e) { interaction.editReply({ embeds: [UI.error("Error al procesar el enlace de Tidal.")] }); }
            }
        ...
        if (command === "playalbum") {
            ...
            if (!query) return interaction.editReply({ embeds: [UI.error("Por favor ingresa el nombre del álbum.")] });

            try {
                const searchRes = await request(`https://itunes.apple.com/search?term=${encodeURIComponent(query)}&entity=album&limit=25`);
                const searchData = await searchRes.body.json();

                if (!searchData.results || searchData.results.length === 0) {
                    return interaction.editReply({ embeds: [UI.error(i18n[l].no_results)] }).then(()=>setTimeout(()=>interaction.deleteReply().catch(()=>{}),MSG_LIFETIME));
                }

                const options = searchData.results.map((album) => {
                    return new StringSelectMenuOptionBuilder()
                        .setLabel(album.collectionName.substring(0, 100))
                        .setDescription(`${album.artistName} • ${album.trackCount} pistas`.substring(0, 100))
                ...
                const albumMsg = await interaction.editReply({ 
                    embeds: [UI.info("Búsqueda de Álbum", `Se encontraron **${searchData.results.length}** resultados para \`${query}\`. Selecciona el correcto en el menú de abajo:`)], 
                    components: [row] 
                });

                const collector = albumMsg.createMessageComponentCollector({ filter: i => i.customId === 'select_album' && i.user.id === userId, time: MSG_LIFETIME });

                collector.on('collect', async i => {
                    const albumId = i.values[0];
                    const selectedAlbum = searchData.results.find(a => a.collectionId.toString() === albumId);
                    
                    const albumName = selectedAlbum.collectionName;
                    const artistName = selectedAlbum.artistName;
                    const coverUrl = selectedAlbum.artworkUrl100.replace('100x100bb', '600x600bb');

                    await i.update({ 
                        embeds: [UI.success(`Procesando álbum oficial: **${albumName}** de **${artistName}**\n> Extrayendo pistas de la base de datos...`)], 
                        components: [] 
                    });

                    try {
                        const tracksRes = await request(`https://itunes.apple.com/lookup?id=${albumId}&entity=song`);
                        const tracksData = await tracksRes.body.json();

                        const tracks = tracksData.results.filter(t => t.wrapperType === 'track');

                        if (tracks.length === 0) {
                            return i.editReply({ embeds: [UI.error("No se pudieron obtener las pistas de este álbum.")] });
                        }

                        await i.editReply({ embeds: [UI.success(`Álbum confirmado: **${albumName}**\n> Añadiendo **${tracks.length} pistas** a la cola. La primera comenzará al instante...`)] });

                        const firstTrack = tracks[0];
                        const r1 = await yts(`${firstTrack.trackName} ${firstTrack.artistName} official audio`);
                        if (r1.videos[0]) {
                            const songObj = createSong(r1.videos[0], firstTrack.artistName, userName, userAvatar);
                        ...
                        const processRestOfAlbum = async () => {
                            for (let j = 1; j < tracks.length; j++) {
                                ...
                                try {
                                    const r = await yts(`${track.trackName} ${track.artistName} official audio`);
                                    if (r.videos[0]) {
                                        const songObj = createSong(r.videos[0], track.artistName, userName, userAvatar);
                        ...
                        i.editReply({ embeds: [UI.error("Ocurrió un error al obtener las canciones del álbum.")] });
                    }
                    setTimeout(() => albumMsg.delete().catch(()=>{}), MSG_LIFETIME);
                });

                collector.on('end', (collected, reason) => {
                    if (reason === 'time' && collected.size === 0) { albumMsg.delete().catch(()=>{}); }
                });

            } catch (e) {
                console.error("[PLAYALBUM ERROR]:", e);
                interaction.editReply({ embeds: [UI.error("Ocurrió un error interno al buscar el álbum.")] }).then(()=>setTimeout(()=>interaction.deleteReply().catch(()=>{}),MSG_LIFETIME));
            }
        }
        ...
        if (command === "song") {
            ...
            try {
                const canvas = createCanvas(1280, 720);
                const ctx = canvas.getContext('2d');
                ctx.quality = 'best'; ctx.patternQuality = 'best'; ctx.imageSmoothingEnabled = true; ctx.imageSmoothingQuality = 'high';
                
                const imgBuffer = await request(q.lastSong.thumbnail).then(res => res.body.arrayBuffer()).then(ab => Buffer.from(ab));
                const mainImage = await loadImage(imgBuffer);
                ...
                ctx.font = '55px sans-serif'; ctx.fillStyle = 'rgba(255,255,255,0.8)';
                ctx.fillText(`${i18n[l].artist}: ${getDisplayArtist(q.lastSong)}`, textX, currentY + 20);
                ctx.font = 'italic 40px sans-serif'; ctx.fillStyle = 'rgba(255,255,255,0.6)';
                ctx.fillText(`${i18n[l].requested_by}: ${q.lastSong.requester}`, textX, 595);

                const attachment = new AttachmentBuilder(canvas.toBuffer(), { name: 'card_musicardi.png' });
                const embedPlay = createNowPlayingEmbed(q.lastSong, currentMs, l, vibrantColorInt);
                const rows = getPlaybackRows(q, l);
                
                await interaction.editReply({ embeds: [embedPlay], components: rows, files: [attachment] });
                startProgressInterval(guildId, vibrantColorInt);

            } catch (error) {
                console.error("[CANVAS ERROR EN /SONG]:", error);
                await interaction.editReply({ embeds: [UI.error("Error al generar el panel visual.")] });
            }
            return;
        }
        ...
        if (command === "lyrics") {
            ...
            if (!queryTitle) {
                if (q.lastSong) {
                    ...
                    queryArtist = cleanArtistName(q.lastSong.realArtist || q.lastSong.artist);
                } else { return interaction.editReply({ embeds: [UI.error(i18n[l].nothing_playing)] }); }
            }

            try {
                const geniusText = await fetchGeniusLyrics(queryTitle, queryArtist);
                
                if (!geniusText) return interaction.editReply({ embeds: [UI.error(i18n[l].lyrics_not_found)] });
                
                const safeText = geniusText.length > 4000 
                    ? geniusText.substring(0, 4000) + "...\n\n*(Letra truncada por límite de Discord)*" 
                    : geniusText;
                
                interaction.editReply({ embeds: [UI.info(`${i18n[l].lyrics_title}: ${queryTitle}`, safeText)] });
            } catch (e) { 
                interaction.editReply({ embeds: [UI.error(i18n[l].no_results)] }); 
            }
        }
        ...
        if (command === "history") {
            ...
            const historyText = q.playedHistory.map((s, i) => `\`${(i + 1).toString().padStart(2, '0')}.\` ${s.title} - ${s.artist}`).join("\n"); 
        ...
        if (command === "autoplay") {
            if (!await checkPremium(guildId)) return interaction.editReply({ embeds: [UI.info("ACCESS DENIED", i18n[l].premium_only)] }).catch(()=>{});
            
            q.autoplay = !q.autoplay;
            if (q.autoplay) q.autoplayCount = 0;
            
            const status = q.autoplay ? "ENABLED" : "DISABLED";
            interaction.editReply({ embeds: [UI.success(`AUTOPLAY: ${status}`)] }).catch(()=>{});
        ...
        if (command === "discovery") {
            ...
            interaction.editReply({ embeds: [UI.success(`DISCOVERY: ${status}`)] }).catch(()=>{});
            
            io.emit("track_added", `DISCOVERY: ${status}`);
            if (q.discovery && !q.playing && q.songs.length === 0 && q.lastSong) playNext(guildId);
            return;
        }
        ...
        if (command === "stop") {
            ...
            interaction.editReply({ embeds: [UI.info(i18n[l].connection, i18n[l].stopped)] }).catch(()=>{});
            setTimeout(() => interaction.deleteReply().catch(()=>{}), MSG_LIFETIME);
            return;
        }
        ...
        if (command === "trivia") {
            ...
            const res = await pool.query('SELECT artist FROM likes GROUP BY artist ORDER BY RANDOM() LIMIT 1');
        ...
        if (command === "language") {
            ...
            return interaction.editReply({ 
                embeds: [UI.info("Configuración de Idioma / Language Setup", "Selecciona el idioma del bot para este servidor:\nSelect the bot language for this server:")], 
                components: [row] 
            });
        }
        ...
        if (interaction.isChatInputCommand() || interaction.isButton()) {
            const errorEmbed = UI.error("Ocurrió un error interno procesando este comando.");
...
client.on(Events.VoiceStateUpdate, (oldState, newState) => {
    ...
    if (botChannel) {
        ...
        if (humans === 0 && !q.stay247) {
            ...
            q.songs = []; q.autoplay = false; q.discovery = false; q.autoplayCount = 0; q.batchCount = 0; q.lastSong = null;
            ...
            q.player.stop(); 
            if (q.connection.state.status !== VoiceConnectionStatus.Destroyed) q.connection.destroy();
            q.connection = null; q.playing = false; q.currentMessage = null; q.filter = null;
            
            if (q.textChannel) {
                const l = guildLangs.get(guildId) || 'es';
                const msg = l === 'es' ? "ECO-MODE: Canal vacío. Reproducción detenida." : "ECO-MODE: Channel empty. Playback stopped.";
                q.textChannel.send({ embeds: [UI.info("SYSTEM", msg)] }).then(m => setTimeout(() => m.delete().catch(()=>{}), MSG_LIFETIME)).catch(()=>{});
            }
        }
    }
});
...
process.on('unhandledRejection', error => { 
    ...
    if (error.message && error.message.includes('Cannot perform IP discovery')) return;
...
io.on("connection", (socket) => {
    ...
    socket.on("get_status", async (req) => {
        ...
        if (guildId && guildId !== "select" && globalQueues.has(guildId)) {
            ...
            if (!data) {
                for (const [id, q] of globalQueues.entries()) {
                    if (q.playing && q.lastSong && q.lastSong.requesterAvatar && q.lastSong.requesterAvatar.includes(userId)) {
                        data = { guildId: id, q };
                        break;
                    }
                }
            ...
            if (!data) {
                const activeQueues = Array.from(globalQueues.entries()).filter(([id, q]) => q.playing);
        ...
        const currentMs = data.q.player.state.resource ? data.q.player.state.resource.playbackDuration + (data.q.lastSong?.seekTime ? data.q.lastSong.seekTime * 1000 : 0) : 0;
        ...
        if (data.q.voiceChannel) {
            data.q.voiceChannel.members.forEach(m => {
                if (!m.user.bot) voiceMembers.push({ id: m.user.id, name: m.user.globalName || m.user.username, avatar: m.user.displayAvatarURL({ extension: 'png', size: 64 }) });
            });
        }
        ...
        try {
            if (data.q.lastSong && data.q.lastSong.videoId) {
                const likeCheck = await pool.query('SELECT 1 FROM likes WHERE user_id = $1 AND video_id = $2', [userId, data.q.lastSong.videoId]);
                isLiked = likeCheck.rowCount > 0;
            }
            
            const statsRes = await pool.query('SELECT listen_time, songs_played FROM user_stats WHERE user_id = $1', [userId]);
            if (statsRes.rows.length > 0) {
                userHours = (statsRes.rows[0].listen_time / 3600).toFixed(1);
                userPlays = statsRes.rows[0].songs_played;
            }
        } catch(e) { console.error("Error al chequear stats/likes:", e.message); }
    ...
    socket.on("cmd_toggle_autoplay", (userId) => {
        ...
        if (data) {
            ...
            io.emit("track_added", `AUTOPLAY: ${data.q.autoplay ? 'ENABLED' : 'DISABLED'}`);
        }
    });
    ...
    socket.on("cmd_toggle_discovery", (userId) => {
        ...
        if (data) {
            ...
            io.emit("track_added", `DISCOVERY: ${data.q.discovery ? 'ENABLED' : 'DISABLED'}`);
        }
    });
    ...
    socket.on("cmd_skip", (userId) => {
        ...
        let activeUserIds = [];
        if (data.q.voiceChannel) data.q.voiceChannel.members.forEach(m => { if (!m.user.bot) activeUserIds.push(m.id); });
        ...
        if (data.q.lastSong && activeUserIds.length > 0) {
            pool.query(`
                ...
                SELECT unnest($1::varchar[]), $2, $3, 0, 1
                ON CONFLICT (user_id, video_id) DO UPDATE SET skips = user_track_stats.skips + 1
            `, [activeUserIds, data.q.lastSong.videoId, data.q.lastSong.title]).catch(()=>{});
        }
    ...
    socket.on("cmd_like", async (userId) => {
        try {
            const data = Array.from(globalQueues.values()).find(q => 
                q.voiceChannel && q.voiceChannel.members.has(userId)
            );
        ...
        } catch (e) { console.error("Error en Like:", e.message); }
    });
    ...
    socket.on("cmd_search", async (query) => {
        try {
            const searchRes = await request(`https://itunes.apple.com/search?term=${encodeURIComponent(query)}&entity=song&limit=6`);
    ...
    const handleRecommendations = async (userId) => {
        try {
            ...
            const randomArtists = allArtistsRes.rows.sort(() => Math.random() - 0.5).slice(0, 3);
            let combinedRecs = [];

            for (const row of randomArtists) {
                try {
                    const searchRes = await request(`https://itunes.apple.com/search?term=${encodeURIComponent(row.artist)}&entity=song&attribute=artistTerm&limit=50`);
                    const data = await searchRes.body.json();

                    if (data.results && data.results.length > 0) {
                        const strictTracks = data.results.filter(t => 
                ...
                } catch (err) { console.error("[ITUNES REC ERROR]", err.message); }
            }
            ...
            socket.emit("recommendations_data", { 
                type: 'personalized', 
                seed: randomArtists.map(r => r.artist).slice(0,2).join(", "), 
                tracks: combinedRecs 
            });
    ...
    socket.on("get_lyrics", async (userId) => {
        try {
            ...
            for (const q of globalQueues.values()) {
                if (q.voiceChannel && q.voiceChannel.members.has(userId)) { targetQueue = q; break; }
            }

            if (!targetQueue || !targetQueue.lastSong) {
                return socket.emit("lyrics_data", { error: "No hay ninguna canción reproduciéndose." });
            }
            ...
            try {
                ...
                if (song.artist) lrclibUrl.searchParams.append("artist_name", song.artist);
            ...
            } catch (e) { console.log("Fallo al conectar con Lrclib:", e.message); }

            if (syncedLyrics) return socket.emit("lyrics_data", { title: song.title, synced: syncedLyrics, lyrics: null });
            if (plainLyrics) return socket.emit("lyrics_data", { title: song.title, synced: null, lyrics: plainLyrics });

            const geniusText = await fetchGeniusLyrics(song.title, song.artist);
            if (geniusText) return socket.emit("lyrics_data", { title: song.title, synced: null, lyrics: geniusText });

            socket.emit("lyrics_data", { title: song.title, error: "Lo sentimos, no pudimos encontrar la letra de esta canción." });

        } catch (error) {
            socket.emit("lyrics_data", { error: "Error interno al buscar la letra." });
        }
    });
    ...
    socket.on("cmd_reorder_queue", ({ userId, guildId, newQueueIds }) => {
        ...
        if (guildId && guildId !== "select" && globalQueues.has(guildId)) {
            ...
            for (const [id, q] of globalQueues.entries()) {
                if (q.voiceChannel && q.voiceChannel.members.has(userId)) { data = { guildId: id, q }; break; }
            }
        }
        ...
        for (let i = 1; i < data.q.songs.length; i++) {
            if (!usedQueueIds.has(i)) { newSongs.push(data.q.songs[i]); }
        }
    ...
    socket.on("cmd_remove_queue", (data) => {
        try {
            ...
            for (const [gId, q] of globalQueues.entries()) {
                if (q && q.voiceChannel && q.voiceChannel.members && q.voiceChannel.members.has(userId)) {
                    targetQueue = q; break;
                }
            }
        ...
        } catch (error) { console.error("[ERROR cmd_remove_queue]:", error.message); }
    });
    ...
    socket.on("cmd_add_queue", async (data) => {
        try {
            ...
            for (const [gId, q] of globalQueues.entries()) {
                if (q && q.voiceChannel && q.voiceChannel.members && q.voiceChannel.members.has(userId)) {
                    targetQueue = q; targetGuildId = gId; break;
                }
            }

            if (!targetQueue || !targetGuildId) { return socket.emit("fallback_to_play", data); }

            const song = {
                title: video.title, artist: video.author || video.artist,
                videoId: video.videoId, url: `https://www.youtube.com/watch?v=${video.videoId}`, 
                thumbnail: `https://i.ytimg.com/vi/${video.videoId}/mqdefault.jpg`,
                requester: userName, requesterAvatar: userAvatar, durationSec: 0 
            };

            targetQueue.songs.push(song);
            if (targetQueue.songs.length === 1 && !targetQueue.playing) { playNext(targetGuildId); }

            socket.nsp.emit("track_added", song.title);

        } catch (error) { console.error("[ERROR cmd_add_queue]:", error.message); }
    });

    socket.on("cmd_play_specific", async ({ userId, guildId, video, userName, userAvatar }) => {
        io.emit("track_added", video.title || "Pista añadida");
        let data = getTargetQueue(userId, guildId);
        let targetGuildId = null; let targetVc = null; let targetTextChannel = null;
        ...
        if (data) {
            ...
            const vcInfo = findUserVoiceChannel(userId);
            if (!vcInfo) return; 
            targetGuildId = vcInfo.guild.id; targetVc = vcInfo.channel;
        ...
        if (!q.connection || q.connection.state.status === VoiceConnectionStatus.Destroyed) {
            if (targetVc) {
                q.connection = joinVoiceChannel({ channelId: targetVc.id, guildId: targetGuildId, adapterCreator: targetVc.guild.voiceAdapterCreator });
                q.connection.on(VoiceConnectionStatus.Ready, () => { q.connection.subscribe(q.player); });
                q.voiceChannel = targetVc; q.textChannel = targetTextChannel || q.textChannel;
            }
        }
        ...
        if (video.videoId && String(video.videoId).startsWith('itunes_')) {
            try {
                const searchRes = await yts(`${video.title} ${video.author} official audio`);
                if (searchRes && searchRes.videos.length > 0) {
        ...
        else if (video.seconds === undefined && video.durationSec === undefined) {
            try {
                const searchRes = await yts(`${video.title} ${video.author || ''} official audio`);
                if (searchRes && searchRes.videos.length > 0) {
                    finalVideo = searchRes.videos[0];
    ...
    socket.on("get_user_guilds", async (userId) => {
        ...
        for (const guild of client.guilds.cache.values()) {
            ...
            } catch(e) { console.error("[GUILD FETCH ERROR]:", e.message); }
        }
    ...
    socket.on("get_preferences", async (userId) => {
        try {
            const res = await pool.query('SELECT * FROM user_preferences WHERE user_id = $1', [userId]);
            if (res.rows.length > 0) {
                socket.emit("preferences_data", res.rows[0]);
            } else {
                socket.emit("preferences_data", { incognito: false, low_res_covers: false, default_filter: 'clear', accent_color: '#a855f7' });
            }
        } catch (e) { console.error("Error get_preferences:", e); }
    });
    ...
    socket.on("save_preferences", async ({ userId, prefs }) => {
        try {
            ...
            `, [userId, prefs.incognito, prefs.low_res_covers, prefs.default_filter, prefs.accent_color]);
        } catch (e) { console.error("Error save_preferences:", e); }
    });
