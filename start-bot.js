...
const tunnel = spawn('npx', ['cloudflared', 'tunnel', '--url', 'http://localhost:3001'], { shell: true });
...
const handleData = (data) => {
    ...
    if (urlMatch) {
        ...
        const rmEnv = spawn('vercel', ['env', 'rm', 'NEXT_PUBLIC_BOT_URL', 'production', '-y', '--cwd', FRONTEND_PATH], { shell: true, stdio: 'inherit' });

        rmEnv.on('close', () => {
            
            
            const addEnv = spawn('vercel', ['env', 'add', 'NEXT_PUBLIC_BOT_URL', 'production', '--cwd', FRONTEND_PATH], { shell: true });
